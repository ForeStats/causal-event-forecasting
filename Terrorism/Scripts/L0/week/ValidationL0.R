...
T_END      <- 8*52
T_HORIZON  <- max(data$week) - 1 - T_END
train_data <- data_grid[data_grid$week <= T_END, ]
test_data  <- data_grid[data_grid$week > T_END & data_grid$week <= T_END + T_HORIZON, ]
...
# TSGLM TRAIN 1D
count_array    <- as.numeric(table(factor(train_data$week, levels = 1:(T_END) )))
tsfit_pois_1d  <- tsglm(ts(count_array), model = list(past_obs = 1:2, past_mean = 1:3), distr = "poisson")
tsfit_pois_1d$logLik

# TSGLM TRAIN 2D
count_array   <- unclass(table(factor(train_data$week, levels = 1:(T_END)), factor(train_data$type, levels = 1:M)))
tsfit_pois_2d <- apply(count_array, 2, FUN = function(z) tsglm(ts(z), model = list(past_obs = 1:2, past_mean = 1:3),
                                               distr = "poisson"))

# TSGLM PREDICT 2D
count_array    <- unclass(table(factor(test_data$week, levels = (T_END + 1):(T_END + T_HORIZON) ), factor(test_data$type, levels = 1:M)))
...
# save data
save.image(file = '~/Documents/MyResearch/Projects/Terrorism/BiharChat/Scripts/workspaceL0.Rdata')

# MODEL PREDICT
load("~/Documents/MyResearch/Projects/Terrorism/BiharChat/Scripts/estimatesL0.RData")
source("helpers.R")
#Parameters
...
truth_array  <- unclass(table( factor(test_data$week, levels = (T_END + 1):(T_END + T_HORIZON)), 
                               test_data$id, factor(test_data$type, levels = 1:M) ))

N_sim <- 500
sim_zk_list = lapply((T_END + 1):(T_END + T_HORIZON), FUN = function(z){
  history_i = model_data[model_data$t <= (z-1)*7]
  
  print(z)
  sim_z_list = lapply(1:N_sim, FUN = function(k){
    sim_k <- event_simulator_L0(start_t = (z-1)*7, t_width = 7, params = params, events = history_i)
    if(nrow(sim_k) > 0){
...
pdf("~/Documents/MyResearch/Projects/Terrorism/BiharChat/Figures/PredPerfL0.pdf", height=3, width=5)
...
theta <- (seq(0, 1, 1/N_PAI)^3)*0.2
zeta <- (seq(0, 1, 1/N_PAI)^3)*0.05
for(i in 1:N_PAI){
  for(j in 1:N_PAI){
...
for(k in 2){
  attacks <- st_as_sf(data[data$week == (T_END + k) & data$type == 3, c('longitude', 'latitude')], 
  ...
  if(nrow(attacks) > 0){
    ...
    pdf(paste0("~/Documents/MyResearch/Projects/Terrorism/BiharChat/Figures/week/GridMapTruth", k, "_L0.pdf"), height=6, width=9)

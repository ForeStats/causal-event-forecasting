...
for(i in 1:100){
  for(j in 1:100){
    model_pais[i,j,]    <- calculate_PAI(truth_array = truth_array, prob_array = model_array_org, 
                                         zone_prop = NULL, mark_prop = NULL, theta = theta[i], zeta = zeta[j])
    baseline_pais[i,j,] <- calculate_PAI(truth_array = truth_array, prob_array = baseline_array, 
                                         zone_prop = zone_prop, mark_prop = mark_prop, theta = theta[i], zeta = zeta[j])
  }
}
m_pais_mat <- matrix(model_pais, 10000, 2)
b_pais_mat <- matrix(baseline_pais, 10000, 2)
m_pais_mat[,1] <- m_pais_mat[,1]/sum(truth_array)
b_pais_mat[,1] <- b_pais_mat[,1]/sum(truth_array)
m_pais_mat[,2] <- m_pais_mat[,2]/(T_HORIZON*Z*M)
b_pais_mat[,2] <- b_pais_mat[,2]/(T_HORIZON*Z*M)
